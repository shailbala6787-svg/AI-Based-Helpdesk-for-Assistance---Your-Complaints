...
export const complaintEmbeddingsDAL = {
  async upsert(complaintId: number, embedding: number[]) {
  ...
  async deleteByComplaintId(complaintId: number) {
    await db
  ...
  async semanticSearch(queryEmbedding: number[], limit: number = 10) {
    ...
    const results = await db
      .select({
        ...
        distance: sql<number>`${complaintEmbeddings.embedding} <=> ${vectorStr}::vector`,
      })
      .from(complaints)
      .innerJoin(
        complaintEmbeddings,
        eq(complaints.id, complaintEmbeddings.complaintId),
      )
