...
export async function parseComplaintImage(base64DataUrl: string): Promise<ParsedComplaint> {
    ...
    input: [
      {
        role: 'user' as const,
        content: [
          { type: 'input_text' as const, text: PARSE_PROMPT },
          { type: 'input_image' as const, image_url: base64DataUrl, detail: 'auto' as const },
        ],
      },
    ],
  });
  ...
  return JSON.parse(jsonMatch[0]) as ParsedComplaint;
}
