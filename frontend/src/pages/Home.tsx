...
export default function Home() {
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-(--color-text) tracking-tight">File a Complaint</h1>
        <p className="text-sm text-(--color-text-muted) mt-2">
          Upload a complaint image to get started. Uploads used: <span className="font-semibold text-(--color-primary)">{user?.uploadsUsed ?? 0}/{user?.uploadLimit ?? 5}</span>
        </p>
      </div>
      ...
      {!draft && (
        <div className="space-y-6">
          ...
          <div
            ...
            className={`relative overflow-hidden border-2 border-dashed rounded-3xl p-14 text-center cursor-pointer transition-all duration-300 shadow-sm ${
              dragOver
                ? 'border-(--color-primary) bg-(--color-primary)/5 scale-[1.02]'
                : 'border-(--color-border-input) hover:border-(--color-primary)/50 bg-white/40 hover:bg-white/60 dark:bg-slate-800/40 dark:hover:bg-slate-800/60 backdrop-blur-md'
            }`}
          >
            {/* Decorative background gradients inside dropzone */}
            ...
            {file ? (
              <div className="flex flex-col items-center gap-3 relative z-10">
                <div className="w-16 h-16 rounded-2xl bg-indigo-100 dark:bg-indigo-900/50 flex items-center justify-center text-indigo-600 dark:text-indigo-400 shadow-inner">
                  <FileImage size={32} />
                </div>
                <div>
                  <p className="text-base font-semibold text-(--color-text)">{file.name}</p>
              ...
              <div className="flex flex-col items-center gap-4 relative z-10">
                <div className="w-16 h-16 rounded-2xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-400 dark:text-slate-500 shadow-inner group-hover:scale-110 transition-transform">
                  <Upload size={32} />
                </div>
                <div>
          ...
          {file && (
            <Button onClick={handleParse} disabled={parsing} className="w-full h-14 text-lg rounded-2xl shadow-md shadow-indigo-500/20">
              {parsing ? (
      ...
      {/* Draft review form */}
      {draft && (
        <form onSubmit={handleSave} className="glass-card shadow-premium rounded-3xl p-8 space-y-5 relative overflow-hidden">
          <div className="absolute top-0 right-0 w-64 h-64 bg-(--color-primary) rounded-full opacity-5 blur-3xl pointer-events-none"></div>

          <div className="flex items-center justify-between mb-4 relative z-10">
            <div>
              <h2 className="text-2xl font-bold text-(--color-text)">Review Details</h2>
              <p className="text-sm text-(--color-text-muted) mt-1">AI has extracted the following information from your image.</p>
            </div>
            <button
              type="button"
              onClick={() => { setDraft(null); localStorage.removeItem(DRAFT_KEY); }}
              className="text-sm font-medium text-red-500 hover:text-red-600 bg-red-50 dark:bg-red-500/10 px-3 py-1.5 rounded-lg transition-colors cursor-pointer"
            >
              Discard
            </button>
          </div>
          ...
          <Button type="submit" disabled={saving} className="w-full h-12 text-md mt-4 relative z-10">
            {saving ? (
              <span className="flex items-center gap-2">
